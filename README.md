...
一个可以部署到公网的小聊天软件。支持注册、登录、添加好友、私聊、未读提示和移动端界面。
...
Render 配置里已经挂载了 1GB 磁盘到 `/var/data`，所以重启后账号和消息不会丢。
...
这是学习版聊天软件，不是生产级微信。公网使用时后续还应该加：HTTPS 强制、验证码、找回密码、消息分页、图片上传、群聊、管理员后台和数据库备份。
